    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="relative w-full h-[180px] flex-shrink-0">
        <Image
          fill
          src={imgSrc}
          alt={listing.listing_heading || "Listing"}
        ...
        <div className="space-y-1.5 mt-auto">
          {typeLabel && (
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-xs text-[#0A3161] font-semibold truncate">
                {typeLabel}
              </p>
            </div>
          )}
    ...
    async function fetchListings() {
      ...
      try {
        const res = await fetch(
          process.env.NEXT_PUBLIC_API_URL + "/api/new-business-listings",
        );
  ...
  return (
    <div className="bg-[#40433F] py-[52px] text-white">
      <div className="container mx-auto px-4">
        <h1 className="text-center xl:text-3xl text-2xl text-white font-bold mb-3">
          New Listings
        </h1>
